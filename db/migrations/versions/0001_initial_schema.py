...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("gastype_oud", sa.CHAR(1), nullable=True),
        sa.Column("gastype_nieuw", sa.CHAR(1), nullable=True),
        sa.Column("bijgewerkt_op", sa.TIMESTAMP(timezone=True), server_default=sa.func.now()),
    )
    ...
    op.create_index(
        "ix_netwerk_tarief_lookup",
        "netwerk_tarief",
        ["version_id", "netbeheerder_code", "energie_type", "klanttype"],
    )
...
def downgrade() -> None:
    ...
    op.drop_table("marktcurve")
    op.drop_index("ix_netwerk_tarief_lookup", table_name="netwerk_tarief")
    op.drop_table("netwerk_tarief")
    op.drop_index("ix_product_component_lookup", table_name="product_component")
    op.drop_table("product_component")
    op.drop_table("vtest_product")
    op.drop_table("data_version")
    op.drop_table("gemeente")
    op.drop_table("netbeheerder")
