...
| `test_dispatch.py` | De brug tussen een fysiek assetmodel en een tijdreeks: zelfconsumptie-eerst-dispatch, per kwartier na te rekenen — overschot laadt, tekort ontlaadt, een volle batterij laat overschot naar injectie gaan. Ook de prijsarbitrage erbovenop: laadt maximaal tijdens de goedkoopste Belpex-uren, verkoopt maximaal tijdens de duurste, blijft zelfconsumptie ertussenin, `marktprijzen=None` raakt het bestaande gedrag niet, en arbitragekoop overschrijdt nooit de bestaande maandpiek. Ook `ac_vermogen_max_w`: een hybride omvormer of plug-and-play batterij (800 W/2500 W of vrij ingevuld) begrenst het laad-/ontlaadvermogen extra — het laagste van die grens en de batterij-nameplate geldt, en een ruimere omvormer dan de batterij verandert niets. |
...
| `test_gebruikers_toml.py` | `gebruiker.toml` blijft in zijn bestaande vorm werken; alles wat erbij komt is optioneel. Inclusief het optionele `[gebruiker].id`: zonder krijgt elke inlezing een nieuwe willekeurige gebruiker, met blijft de identiteit over meerdere inlezingen gelijk, en een ongeldige UUID geeft een duidelijke fout. Ook `[[aansluiting.zonnepaneel]]` (meerdere PV-strings met een eigen `richting`, náást de bestaande enkelvoudige `pv_kwp`-vorm), `[[aansluiting.gastoestel]]` (hangt aan het gaspunt, niet het elektriciteitspunt) en de twee gebouwvelden (`bebouwingstype`/`bewoonbare_oppervlakte_m2`). |
...
| `test_gebruikers_repository.py` | De opslaglaag, op SQLite in het geheugen — de logica is bewust dialectonafhankelijk. `bewaar_simulatie()`/`simulaties()`/`simulatie()` in hun generieke vorm (migratie 0025): herkomst (databankversie, code-commit, dossier-hash/-snapshot) en het volledige `resultaat` blijven leesbaar, en `simulaties()` filtert op gebruiker en scenariotype. Ook migratie 0026: een gastoestel behoudt `vermogen_kw`/`doel`, een PV-string haar `richting`, en een aansluitingspunt zijn gebouwkenmerken. |
| `test_gebruikers_validation.py` | `controleer_dossier()`'s nieuwe gedrag voor `AssetType.GASTOESTEL`: een ontbrekende `vermogen_kw`/`doel` geeft een waarschuwing (geen harde fout — het toestel telt vandaag nog niet mee in de berekening), een volledig toestel geeft er geen, en zonder `hardware`-argument wordt er over gastoestellen niets gezegd. |
...
| `test_scenario_batterij.py` | `BatterijScenario`'s dossiersurgerie en de gedeelde reeksopbouw (`scenario.reeksen`): Fluvius-meting bij voorkeur, SLP-EX als terugval, geen PV geeft een waarschuwing i.p.v. een stille nul, een bestaande PV-installatie wordt niet dubbel geteld via een nieuwe SPP-synthese, prijsarbitrage schakelt zichzelf uit (met waarschuwing) op een niet-overal-dynamisch contract, en `ac_vermogen_max_w` (hybride omvormer/plug-and-play) weigert een niet-positieve waarde en verschijnt in de omschrijving. |
...
| `test_scenario_optimaliseer.py` | `optimaliseer_elektriciteitscontract()`, de "zware calculator" die elk kandidaatcontract afzet tegen het dossier: `kandidaat_contracten()`'s SQL-filters (energievorm, segment, peildatum) en dat hetzelfde product in twee contracttypes (variabel/dynamisch) twee aparte kandidaten oplevert; dat een gefaalde kandidaat de rest niet laat vervallen; en dat een batterij drie modi oplevert (zonder/met batterij/met arbitrage, dat laatste enkel op een dynamisch contract mét marktprijzen) met de drie gevraagde deltas (winst_contractwissel_alleen, winst_batterij_zelfde_contract, winst_gecombineerd) correct berekend. Ook de automatische opslag (`bewaar=True`): een opslagfout verschijnt als waarschuwing zonder het resultaat ongeldig te maken, en `bewaar=False` slaat `_bewaar_optimalisatie()` helemaal over. |
| `test_scenario_herkomst.py` | `scenario.herkomst`: `dossier_snapshot()` bevat geen persoonsgegevens, EAN-code of lokaal bestandspad maar wel de rekenkundig relevante velden; `dossier_hash()` is deterministisch en verschilt bij een ander dossier; `huidige_commit()` geeft `(None, False)` terug buiten een git-repo in plaats van te falen; `scenario_parameters()` geeft exact de dataclass-velden van een scenario, zonder `naam`/`omschrijving`. |
