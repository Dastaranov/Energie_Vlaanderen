...
class TariffPipeline:
    ...
    def process(self, source_path: Path, destination: Path, version_id: str) -> TariffPipelineResult:
        ...
        if target.exists():
            raise TariffPipelineError(f"Tarieven stagingmap bestaat al: {target}")
